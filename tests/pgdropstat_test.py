...
class TestPgDropstat(object):
    ...
    def replace_file(self, file_name):
        sample_config_db_file = os.path.join(test_path, "pgdrop_input", file_name)
        mock_config_db_file = os.path.join(test_path, "mock_tables", file_name)

        #Backup origin config_db and replace it with config_db file with disabled PG_DROP counters
        copyfile(mock_config_db_file, "/tmp/" + file_name)
    ...
    def replace_counter_db_file(self):
        mock_file = self.replace_file("counters_db.json")
    ...
    def replace_config_db_file(self):
        mock_file = self.replace_file("config_db.json")
